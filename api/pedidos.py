...
def criar_pedido(pedido: schemas.PedidoCriar, db: Session = Depends(get_db)):
    ...
    # 2. Abre a comanda vazia
    novo_pedido = models.Pedido(
        id_unidade=pedido.id_unidade,
        id_usuario=pedido.id_usuario,
        canalPedido=pedido.canalPedido,
        status='RECEBIDO', # Status inicial exigido no manual
        valor_total=0.0,
        criado_em=datetime.now()
    )
    db.add(novo_pedido)
    db.commit()
    db.refresh(novo_pedido) # Pega o id da comanda

    # 3. Adiciona os itens e calcula a conta sozinho
    valor_total = 0.0
    ...
    # 4. Fecha a conta com o valor total e salva
