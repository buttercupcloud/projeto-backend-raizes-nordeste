...
def login(requisicao: schemas.LoginRequest, db: Session = Depends(get_db)):
    ...
    if not usuario or not pwd_context.verify(requisicao.senha, usuario.senha_hash):
        raise HTTPException(status_code=401, detail="CPF ou senha inválidos.") # Erro 401 
        
    # 3. Criando o Token JWT válido, inserindo a role dentro dele
    expiracao = datetime.utcnow() + timedelta(hours=2)
    payload = {
        "sub": usuario.cpf,
        "role": usuario.perfil_role, # Verificação de role
        "exp": expiracao
    }
    
    token = jwt.encode(payload, SECRET_KEY, algorithm="HS256")
    
    # Retorno no formato exigido pelo manual
    return {
        "accessToken": token,
        "tokenType": "Bearer",
        "user": {
...
def relatorio_gerencial(authorization: str = Header(default=None)):
    ...
    try:
        ...
        # 2. Descriptografa o token e descobre qual é a role da pessoa
